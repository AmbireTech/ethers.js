...
import { verifyUniversalMessage, verifyTypedData } from "../utils/index.js"
...
describe("Test verify message EOA", function() {
        ...
        it(`sign and verify message`, async function () {
            ...
            const result = await verifyUniversalMessage(wallet.address, humanMessage, signature, provider);
...
describe("Test verify message Smart Wallet", function() {
    ...
    it(`verify message`, async function () {
        const result = await verifyUniversalMessage(signer, humanMessage, signature, provider);
        assert.equal(result, true, "failed to verify message with Smart Wallet");
    })

    it(`verify message with wrong signer`, async function () {
        const result = await verifyUniversalMessage(signer, humanMessage, signatureWithWrongSigner, provider);
...
describe("Test verify Typed-Data EOA (EIP-712)", function() {
    ...
    it(`sign and verify typed-data:`, async function() {
        ...
        if (test?.privateKey && test?.signature) {
            ...
            const result = await verifyTypedData(wallet.address, tests[0].domain, tests[0].types, tests[0].data, sig, provider);
    ...
    it(`sign and verify typed-data with wrang key:`, async function() {
        ...
        if (test?.privateKey && test1?.privateKey ) {
            ...
            const result = await verifyTypedData(wallet.address, tests[0].domain, tests[0].types, tests[0].data, wrangsig, provider);
...
describe("Test verify Typed-Data Smart Wallet (EIP-1271)", function() {
    ...
    it(`verify typed-data`, async function() {
        const result = await verifyTypedData(smartWalletAddr, typedDataSmartWallet.domain, typedDataSmartWallet.types, typedDataSmartWallet.message, typedDataSmartWalletSignature, provider);
    ...
    it(`verify typed-data with wrong signer`, async function() {
        const result = await verifyTypedData(smartWalletAddr, typedDataSmartWallet.domain, typedDataSmartWallet.types, typedDataSmartWallet.message, typedDataSmartWalletSignatureDiffSigner, provider);
